...
#ifndef MNG_INTERFACE
		  info_ptr->iccp_proflen, (unsigned char*)info_ptr->iccp_profile);
#else
		  info_ptr->iccp.proflen, (unsigned char*)info_ptr->iccp.profile);
#endif /* MNG_INTERFACE */
	fprintf(fpout, "}\n");
    }
}
...
/* read and decompile an SNG image presented on stdin */
{
   ...
   if (idat)
   {
       png_set_keep_unknown_chunks(png_ptr, 2, (png_bytep)"IDAT", 1);
