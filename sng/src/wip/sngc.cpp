...
#ifdef _WIN32
#define _CRT_SECURE_NO_WARNINGS 1       // qwell MSVC whining
#define _CRT_SECURE_NO_DEPRECATE 1      // qwell MSVC whining
...
static bool get_inner_token(void)
...
/* collect data in either bitmap format */
{
     ...
     */
    char *bytes = (char*)xalloc(MEMORY_QUANTUM);
    int quanta = 1;
    int	nbytes = 0;
    int ocount = 0;
    int c, maxval = 0;
...
#define P3_FMT		3
	...
	*pbytes = NULL;
	do {
	    ...
	    *pbytes = (char*)xrealloc(*pbytes, *pnbytes + seglen);
	...
	else if (isspace(c))	/* skip whitespace */
	{
		...
		bytes = (char*)xrealloc(bytes, MEMORY_QUANTUM * ++quanta);
...
/* parse IDAT specification and emit corresponding bits */
{
    ...
    free(bits);
#else
    strcpy( (char*)chunk.name, "IDAT");
    chunk.data = (png_byte*)bits;
    chunk.size = nbits;
...
/* parse cHRM specification, set corresponding bits in info_ptr */
{
	...
	{
	    require_or_die("(");
	    wx = (float)double_numeric(get_inner_token());
	    /* comma */
	    wy = (float)double_numeric(get_inner_token());
	    require_or_die(")");
	    cmask |= 0x01;
	}
	...
	{
	    require_or_die("(");
	    rx = (float)double_numeric(get_inner_token());
	    /* comma */
	    ry = (float)double_numeric(get_inner_token());
	    require_or_die(")");
	    cmask |= 0x02;
	}
	...
	{
	    require_or_die("(");
	    gx = (float)double_numeric(get_inner_token());
	    /* comma */
	    gy = (float)double_numeric(get_inner_token());
	    require_or_die(")");
	    cmask |= 0x04;
	}
	...
	{
	    require_or_die("(");
	    bx = (float)double_numeric(get_inner_token());
	    /* comma */
	    by = (float)double_numeric(get_inner_token());
	    require_or_die(")");
	    cmask |= 0x08;
	}
	else
...
{
    png_color_8	sigbits;
    int color = (info_ptr->color_type & (PNG_COLOR_MASK_PALETTE | PNG_COLOR_MASK_COLOR));
...
/* parse gIFx specification and queue up the corresponding chunk */
{
    ...
    memset(&chunk, '\0', sizeof(chunk));
    memset(chunkdata, '\0', sizeof(chunkdata));
    strcpy( (char*)chunk.name, "gIFx");
    chunk.data = chunkdata;

    while (get_inner_token())
	if (token_equals("identifier"))
	{
	    if (string_validate(get_token(), (char*)buf) != 8)
	...
	{
	    if (string_validate(get_token(), (char*)buf) != 3)
...
{
    int		nbytes, bytes_per_sample = 0, nsamples, input_width;
    char	*bytes;
	...
	info_ptr->row_pointers[i] = (png_byte*)&bytes[i * input_width];
...
/* compile a private chunk */
{
	...
	strcpy( (char*)chunk.name, name);

    require_or_die("{");
    collect_data(&nbytes, &bytes);
    require_or_die("}");

    chunk.data = (png_byte*)bytes;
    chunk.size = nbytes;
    png_set_unknown_chunks(png_ptr, info_ptr, &chunk, 1);
